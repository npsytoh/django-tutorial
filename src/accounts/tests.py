...
class AdapterTestCase(TestCase):
    ...
    def test_login_with_profile_user(self):
        from .adapter import MyReportAdapter as adapter
        ...
        self.assertEqual(redirect_url, reverse("report:report-list"))

    #プロフィールが同一のユーザーのテスト
    def test_login_without_profile(self):
        res = self.client.post(signup_url, {"email": "test2@sample.jp",
                                            "password1": self.password,
                                            "password2": self.password})
